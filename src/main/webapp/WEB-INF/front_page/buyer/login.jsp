...
<body>
 ...
 <div class="w loc">
  <div class="h-title" id="logo">
   ...
   <div class="l"
    style="margin: 13px 10px; font-size: 20px; font-family: '微软雅黑'; letter-spacing: 2px">欢迎登录</div>
  </div>
 </div>
 <div class="sign">
  <div class="l ad420x205">
   <a href="#" title="title">
    <img src="/res/img/pic/ppp0.jpg" width="400" height="400" />
   </a>
  </div>
  <div class="r">
   ...
   <!-- 提交登录信息 -->
   <form id="jvForm" action="/front/login.shtml" method="post">

    <input type="hidden" name="returnUrl" value="${param.directUrl}" />
    <ul class="uls form">
     <!-- 显示错误信息 -->
     <li id="errorName" class="errorTip" <c:if test="${error!=null }">style="display: block"</c:if>
      <c:if test="${error==null }">style="display: none"</c:if>>${error}</li>
     <li>
      <!-- 提交用户名 -->
     ...
     <li>
      ...
      <!--验证码图片展示框  -->
      <img src="/CaptchaServlet" onclick="this.src='/CaptchaServlet?d='+new Date()" class="code"
       alt="换一张" align="验证码" />

      <!--使用jq技术 换一张 验证码图片-->
      <a href="javascript:void(0);" onclick="$('.code').attr('src','/CaptchaServlet?d='+new Date())"
       title="换一张">换一张</a>
     </li>
     <li>
      <label for="">&nbsp;</label>
