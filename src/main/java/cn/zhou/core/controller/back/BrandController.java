...
import cn.zhou.common.StaticPage;
...
public class BrandController {
	...
	public String brandList(Integer id, String name, Integer isDisplay, Integer pageNo, Model model) {// brand/list.jsp
		...
		if (isDisplay != null) {
			...
			hashMap.put("isDisplay", isDisplay);
		}
		// 返回服务器地址
	...
	public String divide(String name, Integer isDisplay, Integer pageCode, ModelMap model) {// brand/list.jsp
		...
		hashMap.put("pageCode", Pagination.judgePageCode(pageCode));

		// 设置页面尺寸
		brandBean.setPageSize(5);
		// 设置每页第一条数据索引
		brandBean.setStartIndex(Pagination.getStartIndex(pageCode, 5));
...
//				brandService.query(brandBean)));
		...
		PageBean<BrandBean> pageBean = Pagination.getPageBean(pageCode, brandService.queryTotal(brandBean), parameter,
				brandService.query(brandBean));
		...
		StaticPage.generate("/list.html", "/WEB-INF/freemarker/html", hashMap);

//		return "/brand/list";
		return "forward:/WEB-INF/freemarker/html/list.html";
	}
